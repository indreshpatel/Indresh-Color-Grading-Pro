...
#define SETTING_CURVE_M "curve_m"
#define SETTING_CURVE_R "curve_r"
#define SETTING_CURVE_G "curve_g"
#define SETTING_CURVE_B "curve_b"

// 1. Fixed Unused Parameter error
const char *color_pro_get_name(void *unused) {
    UNUSED_PARAMETER(unused);
    return "Indresh Color Grading";
}
...
void color_pro_get_defaults(obs_data_t *settings) {
    ...
    obs_data_set_default_double(settings, SETTING_CURVE_M, 0.0);
    obs_data_set_default_double(settings, SETTING_CURVE_R, 0.0);
    obs_data_set_default_double(settings, SETTING_CURVE_G, 0.0);
    obs_data_set_default_double(settings, SETTING_CURVE_B, 0.0);
}
...
void *color_pro_create(obs_data_t *settings, obs_source_t *context) {
    ...
    if (filter->effect) {
        ...
        filter->param_curve_m = gs_effect_get_param_by_name(filter->effect, "curve_m");
        filter->param_curve_r = gs_effect_get_param_by_name(filter->effect, "curve_r");
        filter->param_curve_g = gs_effect_get_param_by_name(filter->effect, "curve_g");
        filter->param_curve_b = gs_effect_get_param_by_name(filter->effect, "curve_b");
    }
    obs_leave_graphics();
    bfree(effect_path);
...
void color_pro_update(void *data, obs_data_t *settings) {
    ...
    filter->curve_master.x = (float)obs_data_get_double(settings, SETTING_CURVE_M);
    filter->curve_red.x = (float)obs_data_get_double(settings, SETTING_CURVE_R);
    filter->curve_green.x = (float)obs_data_get_double(settings, SETTING_CURVE_G);
    filter->curve_blue.x = (float)obs_data_get_double(settings, SETTING_CURVE_B);
...
void color_pro_video_render(void *data, gs_effect_t *effect) {
    ...
    gs_effect_set_vec4(filter->param_curve_m, &filter->curve_master);
    gs_effect_set_vec4(filter->param_curve_r, &filter->curve_red);
    gs_effect_set_vec4(filter->param_curve_g, &filter->curve_green);
    gs_effect_set_vec4(filter->param_curve_b, &filter->curve_blue);
...
obs_properties_t *color_pro_get_properties(void *data) {
    ...
    obs_properties_add_button(curve_group, "curve_editor_btn", "Open Graphical Editor (Custom Qt)", [](obs_properties_t*, obs_property_t*) -> bool {
